...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const res = await fetch(`${API_URL}/api/admin/add-sponsor`, {
    method: "POST",
      ...
      <Card className="p-8 shadow-xl border-t-4 border-green-600">
        ...
        <Form method="post" className="space-y-4">
          ...
          <div className="pt-4 border-t">
            ...
            {/* TODO: replace raw company ID with a company dropdown to avoid system-ID entry */}
            <input name="sponsorCompanyId" placeholder="e.g. 101" required className="w-full p-2 border rounded bg-gray-50" />
          </div>
