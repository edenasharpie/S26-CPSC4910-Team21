...
export default function AdminReports() {
  ...
  const fetchReportTypes = async () => {
    try {
      const response = await fetch(`${BASE_URL}/api/admin/reports/types`);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/admin/reports/${selectedReportType}?${queryParams.toString()}`
      );
  ...
  const handleExportPDF = async () => {
    ...
    try {
      setExporting(true);
      const response = await fetch(`${BASE_URL}/api/admin/reports/export`, {
        method: 'POST',
        headers: {
        ...
        <div className="space-y-4">
          ...
          {selectedReport && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t">
              ...
              {selectedReport.allowedFilters.includes('driverId') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Driver ID
                  </label>
              ...
              {selectedReport.allowedFilters.includes('sponsorCompanyId') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Sponsor Company ID
                  </label>
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="e.g., 5"
                    value={filters.sponsorCompanyId || ''}
                    onChange={(e) => handleFilterChange('sponsorCompanyId', e.target.value)}
                  />
                </div>
              )}

              {selectedReport.allowedFilters.includes('status') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Application Status
                  </label>
              ...
              {selectedReport.allowedFilters.includes('orderStatus') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Order Status
                  </label>
              ...
              {selectedReport.allowedFilters.includes('reasonForChange') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Reason for Change
                  </label>
                  <input
                    ...
                    value={filters.reasonForChange || ''}
                    onChange={(e) => handleFilterChange('reasonForChange', e.target.value)}
                  />
                </div>
              )}
            </div>
  ...
  switch (reportType) {
    case 'driver-applications':
      return [
        ...
        { key: 'SponsorCompanyID', header: 'Sponsor Company' },
        {
    ...
    case 'point-transactions':
      return [
        ...
        { key: 'DriverID', header: 'Driver ID' },
        { key: 'UserChanged', header: 'Changed By' },
        {
          key: 'PointChange',
          header: 'Point Change',
          render: (item: any) => {
